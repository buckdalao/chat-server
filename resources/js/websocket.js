var chat = {
  Server: 'reconsitutionfs.com',
  url: 'http://reconsitutionfs.com',
  // 限制socket链接次数
  overflow: false,
  connectTotal: 0,
  timer: null,
...
chat.connectWS = function () {
  if (chat.connectTotal > this.connectLimit) {
    chat.overflow = true
    chat.clearTimer()
    chat.callBack({
      type: 'error',
  ...
  socket = new WebSocket(transport + this.Server + ':' + this.websocketPort)
...
chat.timerFn = () => {
  if (chat.overflow === false && chat.timer === null) {
